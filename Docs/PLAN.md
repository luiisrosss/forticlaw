...
| 11 | Idioma → EN primario + selector ES | next-intl. `/messages/en.json` + `/messages/es.json`. Selector en topbar. Copy de creativos es opción separada por proyecto |
| 12 | Canvas engine → Fabric.js | Editor y export mismo motor. `canvas.toDataURL()` para PNG. Resize a 4 ratios server-side |
| 13 | Templates → JSON en código | `/templates/[nombre].json` en el repo. Sin DB para MVP. 30 templates en lanzamiento |
| 14 | Canvas editor en V1 | Edición post-generación: mover, redimensionar, editar texto, swap imagen, cambiar fondo |
| 15 | Shopify en V1 | OAuth incluido en lanzamiento. Por proyecto. Token cifrado AES-256 en DB |
| 16 | Copy bilingüe EN + ES en V1 | IA genera copy en ambos idiomas. Toggle por proyecto en AI Settings |
| 17 | Sin marca de agua en Free | Free sin watermark. Diferenciador de confianza vs competidores |
...
| 2026-03-15 | Decisiones 11–17 acordadas (idioma, canvas, templates, shopify, copy bilingüe, watermark) | Preparación fase 1 |
| 2026-03-15 | Documentos creados: COMPETITORS.md, FUNCTIONALITY.md, FRONTEND_GUIDE.md | Base documental de la fase 1 |
