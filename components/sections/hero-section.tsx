...
export function HeroSection() {
  ...
  return (
    <section id="waitlist" className="min-h-screen flex flex-col items-center justify-center px-6 pt-24 pb-20 relative">
      ...
      <div className="relative z-10 text-center max-w-3xl mx-auto">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/80 border border-zinc-800 mb-8">
          <span className="text-sm text-zinc-400">{'Now accepting early access applications'}</span>
        </div>

        {/* Headline */}
        <h1 className="font-display text-5xl md:text-7xl font-bold tracking-tight mb-6">
          <span className="text-zinc-100 block">Ad creatives that</span>
        ...
        <p className="text-lg md:text-xl text-zinc-500 max-w-2xl mx-auto mb-10 leading-relaxed text-balance">
          Forticlaw connects to your Shopify store, analyzes your products with AI, and generates scroll-stopping
          Facebook ad creatives in under 60 seconds. Brand kit, copy, and images — all automated.
        </p>

        {/* Waitlist Form */}
        {submitted ? (
          <div className="max-w-md mx-auto p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
            <p className="text-zinc-100 font-medium mb-1">{"You're on the list!"}</p>
            <p className="text-sm text-zinc-500">{"We'll notify you when Forticlaw launches."}</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="max-w-md mx-auto">
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className="flex-1 px-5 py-3 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600 transition-colors text-sm"
              />
              <button
                type="submit"
                className="px-6 py-3 rounded-full bg-zinc-100 text-zinc-900 font-medium text-sm hover:bg-zinc-200 transition-colors whitespace-nowrap"
              >
                {'Get Early Access \u2192'}
              </button>
            </div>
            <p className="text-xs text-zinc-600 mt-4">
              {'Join 1,200+ sellers on the waitlist'}
            </p>
          </form>
        )}
